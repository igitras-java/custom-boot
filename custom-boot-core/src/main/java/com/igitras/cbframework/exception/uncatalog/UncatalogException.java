...
public class UncatalogException extends CustomBootException {
    ...
    public UncatalogException(Exception ex) {
        super(ex.getMessage(), builder().addArguments(ex.getClass(), ex.getMessage())
                .build());
        this.origin = ex;
    }
    ...
    public String getMessage() {
        return String.format("Found uncatalog exception [%s], message: [%s]", origin.getClass()
                .getName(), origin.getMessage());
    }
